...
class Socks5Server:
    def __init__(self, host, port, remote_host, remote_port, logger, backlog=128):
        self.host = host
        self.port = port
        ...
        self.connection_id_sock = {}
        self.sock_connection_id = {}
        self.connection_id = 1

        self.establish_tunnel()

    def establish_tunnel(self):
        self.logger.debug('Trying to connect to remote server: %s:%d' % (self.remote_host, self.remote_port))
        self.remote_sock = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
        self.remote_sock.settimeout(RESEND_TIMEOUT)
        try:
            self.remote_sock.connect((self.remote_host, self.remote_port))
        except Exception as e:
            self.logger.error('Failed to connect to remote server (%s:%d) due to error: %s'
                              % (self.remote_host, self.remote_port, e))

        self.remote_sock.setblocking(0)

        self.logger.info('Tunnel from %s:%d to %s:%d established' %
                         (self.host, self.port, self.remote_host, self.remote_port))

    def tunnel_send_connect(self, connection_id, dest_host, dest_port):
        dest_info = dest_host + ':' + dest_port
        b_dest_info = bytes(dest_info, 'utf-8')
        b_connection_id = connection_id.to_bytes(4, byteorder = 'big')
        b_msg_type = b'C'
        msg_len = len(b_dest_info) + 1 + 4 + 4
        b_msg_len = msg_len.to_bytes(4, byteorder = 'big')
        self.remote_sock.send(b_msg_len)
        self.remote_sock.send(b_msg_type + b_connection_id + dest_info)

    def tunnel_send_buf(self, connection_id, buf):
        b_connection_id = connection_id.to_bytes(4, byteorder = 'big')
        b_msg_type = b'B'
        msg_len = len(buf) + 1 + 4 + 4
        b_msg_len = msg_len.to_bytes(4, byteorder = 'big')
        self.remote_sock.send(b_msg_len)
        self.remote_sock.send(b_msg_type + b_connection_id + buf)
    ...
    def buffer_receive(self, sock):
        ...
        if len(buf) == 0:
            ...
            with self.connection_lock:
                connection_id = self.sock_connection_id[sock]
            self.tunnel_send_buf(connection_id, buf)
    ...
    def flush_and_close_sock_pair(self, sock, error_msg=None):
        ...
        the pair of sockets from connection_id_sock, sock_connection_id. """
        if error_msg:
        ...
        with self.connection_lock:
            partner_sock = self.connection_id_sock.pop(sock)
            self.sock_connection_id.pop(partner_sock)
    ...
    def handle_connect_thread(self, client_sock, addr):
        ...
        self.connection_id_sock and self.sock_connection_id, from where they are serviced by the main loop/thread. """
        ...
        with self.connection_lock:
            ...
            self.connection_id += 1
            self.connection_id_sock[connection_id] = client_sock
            self.sock_connection_id[client_sock] = connection_id

        self.tunnel_send_connect(connection_id, dest_host, dest_port)

        self.logger.debug('Connection to %s:%d established for %s:%d - %d' % (self.remote_host, self.remote_port, dest_host, dest_port, connection_id))
    ...
    def serve_forever(self):
        ...
        while True:
            ...
            with self.connection_lock:
                connected_sockets = list(self.sock_connection_id.keys())
            in_socks = [self.server_sock] + [self.remote_sock] + connected_sockets
...
def main(args):
    ...
    socks5_server = Socks5Server(args.host, args.port, args.remote_host, args.remote_port, logger)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--remote-host', action='store', default='',
                        help='Remote IP/Hostname to serve on', type=str)
    parser.add_argument('--remote-port', action='store', default=1081,
                        help='Remote Port to serve on', type=int)
